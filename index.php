<?php
	// Load settings to get path configurations
	$settings = [];
	if (file_exists('settings.php')) {
		$settings = require 'settings.php';
	} elseif (file_exists('settings.example.php')) {
		$settings = require 'settings.example.php';
	}
	
	// Extract the source URL extension (default to empty string if not set)
	$baseUrl = $settings['paths']['source_url_extension'] ?? '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
	...
	<script>
		const gameConfig = {
			baseUrl: "<?php echo htmlspecialchars($baseUrl); ?>"
		};
	</script>
</head>
<body>
...
<script type="module">
	...
	btnPlay.addEventListener('click', async () => {
		...
		await launchGame({
			lang: selectedLang,
			count: selectedCount,
			baseUrl: gameConfig.baseUrl // Pass the PHP config to the game
		});
	});
</script>
</body>
