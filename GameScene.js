...
class GameScene extends Phaser.Scene {
	constructor() {
		...
		this.topScore = null;
		// REMOVED: The bottomScore manager is no longer needed.
	...
	create() {
		...
		// REMOVED: The BottomScore manager is no longer instantiated.
		this.rightScore = new RightScore(this); // New: Instantiate the accuracy score manager.
		// MODIFIED: BallManager no longer receives a bottomScore reference.
		...
		// MODIFIED: Hint text updated to reflect new key functions.
		this.hintText = this.add.text(
			gameSize.width / 2,
			gameSize.height / 2,
			'USE Q and A keys to increase/decrease Total Max Score', {
				font: '24px monospace',
				fill: '#FFFFFF',
				align: 'center',
				stroke: '#000000',
		...
		this.boardView.init();
		this.topScore.init();
		// REMOVED: bottomScore.init() is no longer called.
		...
		// MODIFIED: The step amount for changing the score is now larger.
		this.input.keyboard.on('keydown-Q', () => this.changeMaxScore(10));
		this.input.keyboard.on('keydown-A', () => this.changeMaxScore(-10));
	}

	// --- MODIFIED: Method now handles changing the TOTAL max score directly. ---
	changeMaxScore(amount) {
		const scoreConfig = GAME_CONFIG.ScoreScenes;

		// Change the total max score, ensuring it doesn't go below 10.
		scoreConfig.TOTAL_MAX_SCORE += amount;
		scoreConfig.TOTAL_MAX_SCORE = Math.max(10, scoreConfig.TOTAL_MAX_SCORE);

		// Update the feedback text.
		this.maxScoreText.setText(`Total: ${scoreConfig.TOTAL_MAX_SCORE}`);
		...
		// Emit an event to notify other managers (TopScore) of the change.
		this.game.events.emit('maxScoreChanged');
		console.log(`Total max score changed to: ${scoreConfig.TOTAL_MAX_SCORE}`);
	}
	...
	handleResize(gameSize) {
		...
		// REMOVED: bottomScore.handleResize() is no longer called.
